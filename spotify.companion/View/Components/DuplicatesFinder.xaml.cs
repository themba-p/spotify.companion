...
{
    public sealed partial class DuplicatesFinder : UserControl
    {
        public DuplicatesFinder()
        {
            ...
            WeakReferenceMessenger.Default.Register<Helpers.ViewMessengerHelper>(this, (r, m) =>
            {
                switch (m.Type)
                {
                    case Enums.ViewHelperType.ScrollDuplicatesToView:
                        object payload = m.Payload ?? ContentView.Items.FirstOrDefault();
                        if (payload != null)
                            ContentView.ScrollIntoView(payload, ScrollIntoViewAlignment.Default);
                        break;
                    default:
                        break;
                }
